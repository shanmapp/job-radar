...
# Asynchronous function to run the crawler for SWBC
async def run_crawler_for_swbc():
    file_path = "urls/urls.json"
    ensure_company_document("SWBC")

    with open(file_path, 'r') as file:
        position_urls = json.load(file)

    swbc_urls = position_urls.get("SWBC", [])
    all_jobs = []
    num_jobs = 20  # You can adjust this as needed

    with ThreadPoolExecutor() as executor:
        tasks = [async_run_crawler(executor, url, num_jobs) for url in swbc_urls]
...
#     asyncio.run(run_crawler_for_swbc())
