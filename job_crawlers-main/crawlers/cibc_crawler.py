...
# Asynchronous function to run the crawler for CIBC
async def run_crawler_for_cibc(receiverEmail=None):
    file_path = "urls/urls.json"
    ensure_company_document("Cibc")

    with open(file_path, 'r') as file:
        position_urls = json.load(file)

    cibc_urls = position_urls.get("Cibc", [])
    all_jobs = []
    num_jobs = 20  # You can adjust this as needed

    with ThreadPoolExecutor() as executor:
        tasks = [async_run_crawler(executor, url, num_jobs) for url in cibc_urls]
...
#     asyncio.run(run_crawler_for_cibc())
